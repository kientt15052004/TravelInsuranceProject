...
public class ClaimsResDBContext extends DBContext {
    ...
    // Method để lấy tất cả claim responses theo claim ID
    ...
    public boolean addClaimResponse(ClaimsRes claimRes) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            ps.setDate(2, new java.sql.Date(claimRes.getCreateDate().getTime()));
            ps.setString(3, claimRes.getDescription());
            ps.setString(4, claimRes.getRelated_img());
            ps.setString(5, claimRes.getRelated_file());
            ps.setString(6, claimRes.getStatus());
